...
<script>
    ...
    // Jalankan lagi saat semua aset (gambar dll) selesai dimuat
    window.addEventListener('load', scrollToBottom);

    // Jalankan berkala sebentar di awal untuk memastikan
    ...
    /* POLLING LOGIC */
    let lastChatHTML = document.getElementById('chatBox').innerHTML;
    
    function fetchMessages() {
        fetch("{{ route('chat.messages', $room) }}")
            ...
            .then(html => {
                if (html !== lastChatHTML) {
                    ...
                    if (isAtBottom) {
                        // Jika sebelumnya di bawah, tetap di bawah
                        scrollToBottom();
                        // Jaga-jaga jika ada media yang dimuat
                        setTimeout(scrollToBottom, 100);
                    } else {
                        // Jika sedang scroll di atas, jangan biarkan mental ke atas (reset ke 0)
